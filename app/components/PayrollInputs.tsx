...
export function TravelFields({ record, onChange, batch = false }: {
  ...
  return <div className="travel-fields">
    <label className="access-check"><input type="checkbox" checked={included} onChange={(event) => {
    ...
    <div className="form-grid form-grid--three">
      ...
      <Field label={batch ? '每条交通费（往返）' : '交通费（往返）'}><NumberInput disabled={!included} value={included ? record.travelFee : 0} placeholder={String(previous?.travelFee ?? 0)} onChange={(travelFee) => edit({travelFee})} /></Field>
    </div>
