    ...
    <section className="content-card profile-editor">
      ...
      <div className="profile-editor__layout">
        <nav className="tab-list" aria-label="资料设置标签">
          <Tab active={tab === 'basic'} onClick={() => changeTab('basic')}>基本信息</Tab>
          {!firstTime && <Tab active={tab === 'documents'} onClick={() => changeTab('documents')}>证件信息</Tab>}
          {!firstTime && <Tab active={tab === 'payment'} onClick={() => changeTab('payment')}>账户信息</Tab>}
          {!firstTime && <Tab active={tab === 'password'} onClick={() => changeTab('password')}>重设密码</Tab>}
        </nav>
    ...
    <FormSection title="基本信息">
      <div className="form-grid form-grid--two">
        <Field label="中文姓" required>
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.lastNameCn} onChange={(event) => setField('lastNameCn', event.target.value)} required />
        </Field>
        <Field label="中文名" required>
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.firstNameCn} onChange={(event) => setField('firstNameCn', event.target.value)} required />
        </Field>
        <Field label="拼音姓">
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.lastNamePinyin} onChange={(event) => setField('lastNamePinyin', event.target.value)} />
        </Field>
        <Field label="拼音名">
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.firstNamePinyin} onChange={(event) => setField('firstNamePinyin', event.target.value)} />
        </Field>
        <Field label="片假名姓">
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.lastNameKana} onChange={(event) => setField('lastNameKana', event.target.value)} />
        </Field>
        <Field label="片假名名">
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.firstNameKana} onChange={(event) => setField('firstNameKana', event.target.value)} />
        </Field>
        ...
        <Field label="现住址" required>
          <textarea rows={3} maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.address} onChange={(event) => setField('address', event.target.value)} required />
        </Field>
        <Field label="联系方式" required hint="可分行填写手机号、微信、邮箱或紧急联系人。">
          <textarea rows={4} maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.tel} onChange={(event) => setField('tel', event.target.value)} required />
        </Field>
      </div>
    </FormSection>
  );
    ...
    <FormSection title="证件信息">
      <div className="form-grid form-grid--two">
        ...
        <Field label="国籍" required={isChinaId}>
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.nationality} onChange={(event) => setField('nationality', event.target.value)} />
        </Field>
        <Field label="证件号" required={isChinaId}>
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.idNumber} onChange={(event) => setField('idNumber', event.target.value)} />
        </Field>
        ...
        {isResidence && (
          <Field label="在留资格" required>
            <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.residentStatus} onChange={(event) => setField('residentStatus', event.target.value)} />
          </Field>
        )}
        {isResidence && (
          <Field label="资格外活动许可">
        ...
        <Field label="证件上住址所在地" required={isChinaId}>
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.addressOfLicense} onChange={(event) => setField('addressOfLicense', event.target.value)} />
        </Field>
        <Field label="（预）毕业院校名">
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.graduateUniversity} onChange={(event) => setField('graduateUniversity', event.target.value)} />
        </Field>
        <Field label="专业名">
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.faculty} onChange={(event) => setField('faculty', event.target.value)} />
        </Field>
        <Field label="（预）毕业时间">
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.graduateDate} onChange={(event) => setField('graduateDate', event.target.value)} />
        </Field>
        <Field label="（预）取得学位">
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.degree} onChange={(event) => setField('degree', event.target.value)} />
        </Field>
      </div>
    </FormSection>
  );
    ...
    <FormSection title="工资收款账户">
      <div className="form-grid form-grid--two">
        ...
        <Field label={isAlipay ? '支付宝账户' : '银行名称'} required>
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.bankName} onChange={(event) => setField('bankName', event.target.value)} />
        </Field>
        {!isAlipay && (
          <Field label={isJapan ? '支店名称' : '开户支行'}>
            <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.bankBranch} onChange={(event) => setField('bankBranch', event.target.value)} />
          </Field>
        )}
        <Field label={isAlipay ? '支付宝账号' : '账号'} required>
          <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.bankAccountNumber} onChange={(event) => setField('bankAccountNumber', event.target.value)} />
        </Field>
        ...
        {(isChina || isAlipay) && draft.payeeIsSelf === '否' && (
          <>
            <Field label="收款人姓名" required>
              <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.payeeName} onChange={(event) => setField('payeeName', event.target.value)} />
            </Field>
            <Field label="收款人中国身份证号">
              <input maxLength={PROFILE_TEXT_MAX_LENGTH} value={draft.payeeIdNumber} onChange={(event) => setField('payeeIdNumber', event.target.value)} />
            </Field>
          </>
        )}
      </div>
      ...
      <FormSection title="重设密码">
        <div className="form-grid form-grid--two">
          ...
          <Field label="旧密码" required>
            <input type="password" maxLength={128} value={oldPassword} onChange={(event) => setOldPassword(event.target.value)} required />
          </Field>
          <Field label="新密码" required>
            <input type="password" minLength={8} maxLength={128} value={newPassword} onChange={(event) => setNewPassword(event.target.value)} required />
          </Field>
          <Field label="确认新密码" required>
            <input type="password" minLength={8} maxLength={128} value={confirm} onChange={(event) => setConfirm(event.target.value)} required />
          </Field>
        </div>
      </FormSection>
