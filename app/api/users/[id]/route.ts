...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    const { id } = await context.params;
    await requireSession(request, id, true);
    const body = await request.json() as { profile?: Profile; expectedProfileVersion?: unknown };
    if (!body.profile) return json({ error: '缺少个人资料。' }, { status: 400 });
    return json({ account: await saveProfile(id, body.profile, typeof body.expectedProfileVersion === 'string' ? body.expectedProfileVersion : undefined) });
  } catch (error) {
    return errorResponse(error);
  }
}
