...
> **当前是试验分支 `ui/review-table-preview`，只供独立本地试用；未合并 main、未部署线上。请勿在本分支执行下方正式部署或重置流程。** 表格功能、验证结果与待确认事项见 [工资审批表格版试用说明](docs/review-table-preview.md)。测试账号和密码只保存在本地 `.local` 目录，不上传仓库。
