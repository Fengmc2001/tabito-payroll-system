...
> **2026-09-14 Logo 与交通费更新：** 左上角使用公司 Logo，点击返回首页。包含交通费的工资只要保存为「未提交」，下次新建时勾选即可回填，无需提交审核。修改范围、验证结果见 [更新说明](docs/logo-travel-2026-09-14.md)。本轮无数据库迁移、不重置数据，只更新本地预览和 main。

> **2026-09-14：工资审批表格版已获确认，纳入 main。** 审批使用单一的行内展开详情，管理员可在详情底部的「管理员操作」中分配或转交审核；工资申报不新增转交入口。本轮不部署线上、不清空数据库、不新增重置标记。功能与验证边界见 [表格版说明](docs/review-table-preview.md)。测试账号和密码只保存在本地 `.local` 目录，不上传仓库。

> **2026-09-13 更新注意：本次首次服务器部署会重新初始化一次。** 请先备份服务器数据库和附件，再在交互终端运行 `npm run deploy:cloudflare` 并确认 `y`。即使已做过旧版重置，本次仍将清除旧账号、工资、资料、权限及附件。完成后后续更新保留数据；本地启动不清库。首管理员仍固定为 `TabitoAdimin01@tabitoedu.com`，需预设的 `BOOTSTRAP_SECRET` 和自行设置的新密码。更新步骤见 [本次发布说明](docs/release-2026-09-13.md)。
...
- 资料校验：生日必须是四位年份的有效日期；支持 My Number Card，My Number 番号推荐填写但不必填；登录身份证件、资格外活动许可和抚养资料均为选填；银行卡/支付宝账户截图推荐上传，允许不上传，最多 2 个附件。
- 工资申报：默认显示当前自然月，也可选择其他月份；未提交、待审核、已驳回、已通过完整保留，提交动作只处理所选月份的草稿。
- 默认时区统一为日本 `Asia/Tokyo`（UTC+9）：今日、默认月份、年度、定期申报和灰度样例按日本日历计算，登录、附件与审批/审计时间按日本时间显示，不依赖浏览器或服务器的本地时区。数据库时间戳仍以 UTC 保存，历史工作日期与金额不变。
...
- 按月审批：管理员默认查看当月全部账号，审核员只查看指定交给自己的记录；也可按账号单独查看；账号、月份和状态可组合筛选，顶部金额随当前账号范围更新。待审、通过、驳回同时保留在工作月份内，审批后不会从当月列表消失。
- 工资代报：仅管理员可为其他员工新建单条申报或批量排班；所有角色均可为本人批量申报；一次最多生成 62 条，提交失败时不会留下半批数据。同一批次使用 `requestId` 保证安全重试，不会重复创建。
- 左侧申报导航：所有账号显示“本人申报 / 本人多条申报”，只有管理员额外显示“他人单条申报 / 他人多条申报”。
- 定期申报：批量排班可保存为按月规则，支持暂停、恢复、删除和手动生成。Cloudflare 定时任务会分页处理到期规则，同一规则同一月份只生成一次。
- 审批来源可追踪：本人申报、单条代报、批量代报和定期生成都会保存工资所属人、创建人、提交人、批次及规则来源。管理员可审批全部待审记录；审核员可审批指定给自己的记录（包括本人申报）。拥有完整资料与审批授权的账号，可另外审批所选员工全部待审记录，最终付款前仍由人工核对。
- 并发保护：本人草稿、代报草稿、账号权限和定期规则使用版本校验；整月提交、批量代报和关键审计写入使用 D1 原子批处理，过期页面不能覆盖较新的修改。
- `04 工资审核`：分为“工资审批”和“工资汇总”。汇总可展开到员工当月每条工资、附件和审批历史；默认审核员仅统计本人负责的部分，不含他人的月总收入和收款资料。管理员及获完整资料授权的账号，可在授权范围查看收款资料并导出 Excel。
- `06 员工管理`：管理员默认可用；其他账号需同时开通功能与对应员工的完整资料查看权限。授权只读，不含未提交草稿。
- `07 总审计`：当月/年度已审批支出、待审/驳回、部门分解；选择员工后，月度、年度、逐月及部门汇总都会按该账号过滤，并显示该账号当月操作追踪。
- 页面审计最多显示最近 10 条且按当前查看权限过滤；审批附件、历史附件和直接下载接口采用相同的数据权限。操作人优先显示姓名，缺失时回退到邮箱。
- 导航、提醒、审计指标、附件和转账操作使用语义一致的 SVG 图标，减少阅读纯文本的时间；图标来自 [Lucide](https://lucide.dev/)（ISC License）。

## 2026-09-10 权限调整

管理员在“账号与权限 → 查看与审核配置”中选择账号，分别设置功能入口、完整资料与工资审批授权和负责人对应审核员。**自 2026-09-13 本次调整起，完整资料授权同时允许通过或驳回所选员工的全部待审工资，不受负责人和指定审核员限制，普通员工账号也适用。** 原有授权同样生效；工资审批入口自动开放。代报、转交审核、账号管理仍仅管理员可用。撤销授权不需要重新登录；若仍是其他员工某条工资的指定审核员，保留该条审批权。非管理员自审必须单独勾选本人授权，取消后即时撤销自审权。详见 [完整资料与审批授权说明](docs/approval-grants-2026-09-13.md)。

数字输入、交通费开关、手机横向工资表、申报截止提醒及作废流程见 [工资申报与作废更新说明](docs/payroll-ux-and-void-2026-09-13.md)。作废保留原审批和附件，已通过工资仅管理员填写原因并核对付款后可作废。此次界面更新不新增清库批次，沿用已有一次性发布规则。

2026-09-14：审核员分配改为“明确配置优先，未配置时由具备审核角色的负责人本人审核”，继续检查自审授权。已有待审记录不自动改派；同时修复交通费默认值、计费选项顺序、弹窗布局并增加日期点选。详见 [审核分配与界面更新](docs/reviewer-routing-2026-09-14.md)。

本次新增迁移 `0007`、`0008`，仅扩展结构，不清空或重算工资。旧记录未指定审核员时由管理员处理；旧审核员建立的他人自动规律停止执行，可由管理员明确接管。既有的一次性正式部署重置规则未变更，本次没有执行线上重置。

操作说明、权限矩阵和验收记录见 [权限调整说明](docs/permissions-2026-09-10.md)。
...
npm run self-check:timezone
npm run self-check:audit-fixes
npm run self-check:deployment
npm run self-check:delegated-ui
npm run self-check:permissions-migration
npm run build
npm audit --omit=dev
```

后端权限自检必须对一次性本地数据库运行，并按后台 → 代报 → 权限专项的顺序执行。代报测试会创建第二个管理员，之后不要在同一库重跑“最后管理员”测试：
...
PAYROLL_TEST_ADMIN_PASSWORD='set-a-test-password' \
PAYROLL_TEST_BOOTSTRAP_SECRET='SelfCheckBootstrap2026!' \
npm run self-check:permissions
```
